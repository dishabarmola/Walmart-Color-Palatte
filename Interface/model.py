...
def detect_colors(image, num_colors=20, detect_lip_color=False):
    ...
    filtered_colors = [tuple(map(int, color)) for color in colors if not (color[0] == color[1] == color[2] or
    ...
    if detect_lip_color:
        ...
        if (lip_roi_start_x < image.shape[1] and lip_roi_start_y < image.shape[0] and
            ...
            st.warning("The defined ROI for lip detection is out of bounds of the image.")

    if len(filtered_colors) < num_colors:
        num_additional_colors = num_colors - len(filtered_colors)
...
def recommend_colors(season):
    ...
    url = f"https://www.walmart.com/c/kp/{colors}-clothes"
    color_ranges = {
...
def main():
    ...
    if uploaded_file is not None:
        ...
        if cropped_image is not None:
            ...
            st.markdown(
                f'<a href="{recommended_colors_url}" target="_blank" class="button">Check out the recommended products on Walmart</a>',
            ...
            if recommended_colors_palette:
                ...
                for i, color in enumerate(recommended_colors_palette):
                    color_palette_image[:, i * 100:(i + 1) * 100] = color
                st.image(color_palette_image, caption="Color Palette", use_column_width=True)
